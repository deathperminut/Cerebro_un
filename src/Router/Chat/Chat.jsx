...
export default function Chat() {
  ...
  const loadHistorial =async()=> {
    ...
    setPreloader(true);
    result = await loadChatHistorial().catch((error)=>{
      console.log(error);
      setPreloader(false);
      Swal.fire({
        icon: 'info',
        title: 'Problemas para cargar historial de chats'
      });
    })

    if(result){
      console.log(result.data['info_chats']);
      setPreloader(false);
      setChatHistory(OrganizeChats(result.data['info_chats']).reverse())
    }

  }


  const loadHistorial_promise =()=> {

    loadChatHistorial().then((result)=>{
      console.log(result.data['info_chats']);
      setChatHistory(OrganizeChats(result.data['info_chats']).reverse())
    }).catch((error)=>{
      console.log(error)
  ...
  const loadHistorial_new=(IdChat,Conv)=>{
    // creamos el objeto
    let objectChat = {
    ...
    let copy_history =  [...chatHistory];
    copy_history.unshift(objectChat);
    setChatHistory(copy_history);
  ...
  const oldChatAccess=(chat)=>{
    setIdChat(chat.id);
    setNewChat(false);
    setChatSelect(true);
    // NOS TRAEMOS LAS CONVERSACIONES CON EL ID;
    getOldConversations(chat.id);
  }

  const getOldConversations=async(chatId)=>{

    let result = undefined;
    
    result =  await getConversations({'id_chat':chatId}).catch((error)=>{
      console.log(error);
      setStatusBrain('Online');
      Swal.fire({
        icon: 'info',
        title: 'Problemas para cargar conversación'
      });
    })
    ...
    if(result){
      setPreloader(false);
      setStatusBrain('Online');
      setConversation(OrganizeConversation(result.data.information_chat));
      scrollToBottom()
    }
  ...
  const appendConversationAI=(conv)=>{
    let lista_copia = [...conversation];
    lista_copia.push({
      position:"left",
      type:"text",
  ...
  const prueba=(event)=>{
    console.log("VALOR: ",inputReferance.current.value);
  }
  ...
  const DeleteChat=async()=>{
    ...
    let result =  undefined;
    setPreloader(true);
    result =  await deleteChat({'id_chat':idChat}).catch((error)=>{
      console.log(error);
      setPreloader(false);
      Swal.fire({
        icon: 'info',
        title: 'Problemas para eliminar la conversación'
      });
    })

    if(result.data['result_request'] === 'La conversación ha sido eliminada correctamente.'){
      // mostramos la pestaña inicial del chat
      setChatSelect(false);
      setIdChat(null);
      setConversation([]);
      setNewChat(false);
      // cargamos el historial de conversaciones
      loadHistorial();
    }else{
      console.log(result.data['result_request']);
      setPreloader(false);
      Swal.fire({
        icon: 'info',
        title: 'Problemas para eliminar la conversación'
                  ...
                  left={
                    <div className='d-flex flex-row justify-content-start align-items-center align-self-center'>
                      ...
                      <div className='d-flex flex-column'>
                        <p className='m-0 lh-sm fs-5- ff-monse-regular- fw-bold tx-black-' style={{ alignSelf: "start" }}> Cerebro UN </p>
                        <p className='m-0 lh-sm fs-6- ff-monse-regular- fw-normal tx-black-' style={{ alignSelf: "start" }}>{statusBrain}</p>
                      </div>
                      {newChat==false ? 
                      <div className='d-flex flex-column'>
                        <div className='buttonElementDelete' onClick={DeleteChat}>
                                <MdOutlineDeleteOutline className='m-0 lh-sm fs-5- ff-monse-regular- fw-bold tx-black-' style={{ alignSelf: "center" }} color='white'></MdOutlineDeleteOutline>
                        </div>
                      </div>
                      :
                      <></>
